...
function pandocMarkdownToHTML(settings, keys) {
  return __async(this, null, function* () {
    ...
    if (!settings.pathToBibliography) {
      throw new Error("Error: pandocMarkdownToHTML bibliography file is required");
    }
    const args2 = [
      "-f",
      "markdown",
...
function pandocHTMLToBibFragment(html) {
  if (!html) {
    throw new Error("Error: pandocHTMLToBibFragment received empty HTML string");
  }
  let parsed = new DOMParser().parseFromString(html, "text/html");
  const refs = parsed.getElementById("refs");
  if (!refs) {
    throw new Error("Error: pandocHTMLToBibFragment references container not found");
  }
  parsed = null;
  return refs;
}
  ...
  getReferenceList(file, content) {
    return __async(this, null, function* () {
      ...
      if (!cachedDoc || !areSetsEqual(cachedDoc.keys, citeKeys) || cachedDoc.cslStyle !== cslStyle || cachedDoc.pathToBibliography !== pathToBibliography) {
        try {
          const bib = pandocHTMLToBibFragment(yield pandocMarkdownToHTML(__spreadProps(__spreadValues({}, this.plugin.settings), {
            cslStyle,
            pathToBibliography
          }), citeKeys));
