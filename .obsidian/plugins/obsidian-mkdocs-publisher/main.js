...
var MkdocsSettingsTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Category Key").setDesc("The frontmatter key to set the category of your file.").addText((text) => text.setPlaceholder("category").setValue(this.plugin.settings.categoryKey).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.categoryKey = value.trim();
      yield this.plugin.saveSettings();
    })));
    new import_obsidian.Setting(containerEl).setName("Default category").setDesc("The default folder where you note will be published.").addText((text) => text.setPlaceholder("Notes").setValue(this.plugin.settings.categoryDefault).onChange((value) => __async(this, null, function* () {
    ...
    new import_obsidian.Setting(containerEl).setName("Index Folder Note Key").setDesc(desc_index).addText((text) => text.setPlaceholder("(i)").setValue(this.plugin.settings.indexFolder).onChange((value) => __async(this, null, function* () {
...
// mkdocsPublisher/utils/publication.ts
...
var MkdocsPublish = class {
  ...
  publish(file, one_file = false) {
    return __async(this, null, function* () {
      ...
      try {
        ...
        yield this.uploadText(file.path, text, file.name);
  ...
  uploadFolder() {
    return __async(this, null, function* () {
      const folder = yield this.getSharedFiles();
      if (folder.length > 0) {
        const publishedFiles = folder.map((file) => file.name);
        const publishedFilesText = JSON.stringify(publishedFiles).toString();
        yield this.uploadText("vault_published.json", publishedFilesText, "vault_published.json");
      }
    });
  }
  upload(filePath, content, title) {
    return __async(this, null, function* () {
      ...
      if (!this.settings.githubName) {
        new import_obsidian2.Notice("Config error : You need to define your github username in the plugin settings");
        throw {};
      }
  ...
  uploadImage(imageFile) {
    return __async(this, null, function* () {
      ...
      const image64 = (0, import_obsidian2.arrayBufferToBase64)(imageBin);
      yield this.upload(imageFile.path, image64, imageFile.name);
    });
  }
  uploadText(filePath, text, title = "") {
    return __async(this, null, function* () {
      try {
        const contentBase64 = gBase64.encode(text).toString();
        yield this.upload(filePath, contentBase64, title);
      } catch (e) {
        console.error(e);
      }
    });
  ...
  updateSettings() {
    return __async(this, null, function* () {
      ...
      try {
        yield this.uploadText(".github-actions", newSettings, ".github-actions");
        return true;
      } catch (e) {
        return false;
      }
...
// mkdocsPublisher/main.ts
var MkdocsPublication = class extends import_obsidian3.Plugin {
  onload() {
    return __async(this, null, function* () {
          ...
          menu.addItem((item) => {
            item.setTitle('Share "' + file.basename + '" with Mkdocs Publisher').setIcon("share").onClick(() => __async(this, null, function* () {
              try {
                ...
                const publishSuccess = yield publish.publish(file, true);
                if (publishSuccess) {
                  new import_obsidian3.Notice('Send "' + file.basename + '" to ' + this.settings.githubRepo + ".\nNow, waiting for the workflow to be completed...");
                  yield publish.workflowGestion();
                  new import_obsidian3.Notice("Successfully published " + file.basename + " to " + this.settings.githubRepo + ".");
                }
              } catch (e) {
                console.error(e);
                new import_obsidian3.Notice("Error publishing to " + this.settings.githubRepo + ".");
              }
            }));
          });
          menu.addSeparator();
        ...
        if (disablePublish(this.app, this.settings, view.file) && this.settings.editorMenu) {
          ...
          menu.addItem((item) => {
            item.setTitle('Share "' + view.file.basename + '" with Mkdocs Publisher').setIcon("share").onClick(() => __async(this, null, function* () {
              try {
                const publish = new MkdocsPublish(this.app.vault, this.app.metadataCache, this.settings);
                const publishSuccess = yield publish.publish(view.file, true);
                if (publishSuccess) {
                  new import_obsidian3.Notice('Send "' + view.file.basename + '" to ' + this.settings.githubRepo + ".\nNow, waiting for the workflow to be completed...");
                  yield publish.workflowGestion();
                  new import_obsidian3.Notice("Successfully published " + view.file.basename + " to " + this.settings.githubRepo + ".");
                }
              } catch (e) {
                console.error(e);
                new import_obsidian3.Notice("Error publishing to " + this.settings.githubRepo + ".");
          ...
          if (disablePublish(this.app, this.settings, this.app.workspace.getActiveFile())) {
            if (!checking) {
              try {
                ...
                if (publishSuccess) {
                  new import_obsidian3.Notice('Send "' + currentFile.basename + '"to ' + this.settings.githubRepo + ".\nNow, waiting for the workflow to be completed...");
                  publishFile.workflowGestion();
                  new import_obsidian3.Notice("Successfully published " + currentFile.basename + " to " + this.settings.githubRepo + ".");
                }
              } catch (e) {
                console.error(e);
                new import_obsidian3.Notice("Error publishing to mkdocs.");
              }
            }
            return true;
          }
      ...
      this.addCommand({
        ...
        callback: () => __async(this, null, function* () {
          ...
          try {
            ...
            const sharedFiles = yield publish.getSharedFiles();
            ...
            if (sharedFiles.length > 0) {
              ...
              const publishedFilesText = JSON.stringify(publishedFiles).toString();
              yield publish.uploadText("vault_published.json", publishedFilesText, "vault_published.json");
              for (let files = 0; files < sharedFiles.length; files++) {
                try {
                  const file = sharedFiles[files];
                  statusBar.increment();
                  yield publish.publish(file);
                } catch (e) {
                  errorCount++;
                  new import_obsidian3.Notice(`Unable to publish note ${sharedFiles[files].name}, skipping it`);
                }
              }
              statusBar.finish(8e3);
              new import_obsidian3.Notice(`Send ${publishedFiles.length - errorCount} notes to ${this.settings.githubRepo}`);
              yield publish.workflowGestion();
              new import_obsidian3.Notice(`Successfully published ${publishedFiles.length - errorCount} notes to ${this.settings.githubRepo}.
Now, waiting for the workflow to be completed...`);
            }
          } catch (e) {
            console.error(e);
            new import_obsidian3.Notice("Unable to publish multiple notes, something went wrong.");
          }
        })
      });
      this.addCommand({
        ...
        callback: () => __async(this, null, function* () {
          try {
            const { vault, metadataCache } = this.app;
            const publish = new MkdocsPublish(vault, metadataCache, this.settings);
            const successUpdate = yield publish.updateSettings();
            if (successUpdate) {
              new import_obsidian3.Notice("Successfully updated " + this.settings.githubRepo + "settings.");
            }
          } catch (e) {
            console.error(e);
            new import_obsidian3.Notice("Unable to update settings, something went wrong.");
          }
        })
      });
    });
